...
import com.google.gwt.gears.persistence.mapping.MappingException;
 ...
 * @author Alex Bertram
 */
public class PersistenceUnitCompiler {
  ...
  public <T extends PersistenceUnit> T create(Class<T> contextClass) {
    ...
    try {
      ...
      gtor.writeClasses(new MockLogger(), new MockGeneratorContext(), mapping);

      DiagnosticCollector<JavaFileObject> diagnostics = new DiagnosticCollector<JavaFileObject>();

      StandardJavaFileManager jfm = compiler.getStandardFileManager(diagnostics, null, null);
      jfm.setLocation(StandardLocation.CLASS_OUTPUT, Collections.singleton(new File("target/test-classes")));
      ...
      Class<T> context = (Class<T>) Class.forName(mapping.getPersistenceContextImplQualifiedClass());
