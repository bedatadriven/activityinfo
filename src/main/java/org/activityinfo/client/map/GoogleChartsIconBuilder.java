...
/**
 * This gives you static functions for creating dynamically sized and colored
 * marker icons using the Google Charts API marker output.
 */
public class GoogleChartsIconBuilder {
     ...
     * blank space on the side, depending on the height of the icon, as the icon
     * will scale its shape proportionately
     */
    ...
    /**
     * Specifies, as a hexadecimal string, the color used for the majority of
     * the icon body.
     */
    ...
    /**
     * Specifies, as a hexadecimal string, the color used for the top corner of
     * the icon. If you'd like the icon to have a consistent color, make the
     * this the same as the <code>primaryColor</code>.
     */
    ...
    /**
     * Specifies, as a hexadecimal string, the color used for the shadow of the
     * icon.
     */
     ...
     * Generally, one or two characters looks best.
     */
    ...
    /**
     * Specifies, in pixels, the size of the label text. If set to 0, the text
     * auto-sizes to fit the icon body.
     */
    ...
    /**
     * Specifies shape of the icon. Current options are "circle" for a circle or
     * "roundrect" for a rounded rectangle.
     */
    ...
    /**
     * Creates an icon based on the specified options Supported options are:
     * width, height, primaryColor, strokeColor, and cornerColor.
     * 
    ...
    public Icon createMarkerIcon() {
        ...
        icon.setInfoWindowAnchor(Point.newInstance(width / 2,
            (int) Math.floor(height / 12)));
    ...
    public Icon createPinUrl() {
        ...
        icon.setInfoWindowAnchor(Point.newInstance(21 / 2,
            (int) Math.floor(34 / 12)));
        icon.setImageMap(new int[] {
            width / 2, height,
            (int) ((7d / 16d) * width), (int) ((5d / 8d) * height),
            (int) ((5d / 16d) * width), (int) ((7d / 16d) * height),
    ...
    /**
     * Creates a flat icon based on the specified options Supported options are:
     * width, height, primaryColor, shadowColor, label, labelColor, labelSize,
     * and shape..
     * 
    ...
    public Icon createFlatIcon() {
        ...
        if (!"circle".equals(shape)) {
            icon.setImageMap(new int[] { 0, 0, width, 0, width, height, 0,
                height });
        } else {
    ...
    public static int[] createCircleImageMap(int width, int height,
        int polyNumSides) {
