...
public class GoogleMapsReportOverlays {
    ...
    public Extents addMarkers(List<MapMarker> markers) {
        ...
        for (MapMarker marker : markers) {
            ...
            LatLng latLng = LatLng
                .newInstance(marker.getLat(), marker.getLng());
    ...
    public void setView(MapContent content) {
        ...
        mapWidget.setCenter(LatLng.newInstance(content.getCenter().getLat(),
            content.getCenter().getLng()));
    }
    ...
    protected void addAdminOverlays(AdminOverlay adminOverlay,
        ...
        for (int i = 0; i != geometry.getEntities().length(); ++i) {
            ...
            AdminMarker marker = adminOverlay.getPolygon(entityGeometry
                .getAdminEntityId());
            if (marker != null) {
    ...
    private EncodedPolyline[] toPolylines(AdminEntityGeometry entityGeometry,
        int zoomFactor, int numLevels) {
        EncodedPolyline[] polylines = new EncodedPolyline[entityGeometry
            .getPolygons().length()];
