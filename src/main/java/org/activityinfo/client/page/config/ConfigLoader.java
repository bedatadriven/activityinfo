...
public class ConfigLoader implements PageLoader {
    ...
    private Map<PageId, Provider<? extends Page>> pageProviders = Maps
        .newHashMap();
    ...
    public ConfigLoader(
        ...
        placeSerializer.registerStatelessPlace(DbListPresenter.PAGE_ID,
            new DbListPageState());
        placeSerializer.registerParser(DbConfigPresenter.PAGE_ID,
            new DbPageState.Parser(DbConfigPresenter.PAGE_ID));
        placeSerializer.registerParser(DbUserEditor.PAGE_ID,
            new DbPageState.Parser(DbUserEditor.PAGE_ID));
        placeSerializer.registerParser(DbPartnerEditor.PAGE_ID,
            new DbPageState.Parser(DbPartnerEditor.PAGE_ID));
        placeSerializer.registerParser(DbProjectEditor.PAGE_ID,
            new DbPageState.Parser(DbProjectEditor.PAGE_ID));
        placeSerializer.registerParser(LockedPeriodsPresenter.PAGE_ID,
            new DbPageState.Parser(LockedPeriodsPresenter.PAGE_ID));
        placeSerializer.registerParser(DesignPresenter.PAGE_ID,
            new DbPageState.Parser(DesignPresenter.PAGE_ID));
        placeSerializer.registerParser(DbTargetEditor.PAGE_ID,
            new DbPageState.Parser(DbTargetEditor.PAGE_ID));
        placeSerializer.registerStatelessPlace(IndicatorLinkPage.PAGE_ID,
            new IndicatorLinkPlace());
    }
    ...
    @Override
    public void load(final PageId pageId, final PageState place,
            ...
            public void onSuccess() {
                ...
                if (page == null) {
                    callback.onFailure(new Exception(
                        "ConfigLoader didn't know how to handle "
                            + place.toString()));
                } else if (page instanceof DbPage) {
                    dispatch.execute(new GetSchema(),
                        new AsyncCallback<SchemaDTO>() {
                            ...
                            public void onSuccess(SchemaDTO result) {
                                ...
                                ((DbPage) page).go(result
                                    .getDatabaseById(dbPlace.getDatabaseId()));
