 ...
 * {@link org.activityinfo.client.page.PageState}s into/from history tokens.
 * 
...
public class PageStateSerializer {
    ...
    public PageState deserialize(String token) {
        // If an ID separator is found ('!'), uses it instead of the path
        // separator
        int i = token.indexOf('!');
        if (i == -1) {
            i = token.indexOf('/');
        ...
        if (parser != null) {
            ...
            Log.warn("PlaceDeserializer: No parser registered for page '"
                + pageId + ".");
    ...
    public void registerParser(PageId pageId, PageStateParser parser) {
        ...
        Log.debug("PageSerializer: registered page serializer "
            + parser.toString() + " for pageId '" + pageId + "'");
    }
