...
/**
 * End-point for Open Refine (previously Google Refine) reconciliation service.
 * This is allows you to use Google Refine to match existing data to
 * administrative entities and locations within ActivityInfo's geographic module
 * and import that data into AI.
 * 
...
public class ReconciliationService {
    ...
    public Response getMetadata(@Context UriInfo uri,
        @QueryParam("callback") String callback) {
        ServiceDescription description = new ServiceDescription(
            uri.getBaseUri());

        if (Strings.isNullOrEmpty(callback)) {
            return Response.ok(description).type(MediaType.APPLICATION_JSON)
                .build();
        } else {
            return Response.ok(new JSONWithPadding(description, callback))
    ...
    public Object reconcile(MultivaluedMap<String, String> formParams)
        throws JsonProcessingException, IOException {
        ObjectMapper mapper = new ObjectMapper();
        if (!formParams.get("queries").isEmpty()) {
            Map<String, Query> batch = mapper.readValue(
                formParams.getFirst("queries"),
    ...
    private Map<String, QueryResponse> batchReconcile(Map<String, Query> batch)
    ...
    private List<AdminEntity> queryByLevel(int levelId) {
        ...
        entities = entityManager
            .get()
            .createQuery(
                "select e from AdminEntity e where e.level.id = :levelId")
    ...
    private List<AdminEntity> searchBySound(String queryText) {
        ...
        List<AdminEntity> entities = entityManager
            .get()
            .createQuery(
                "select e from AdminEntity e where e.soundex = :soundex")
            .setParameter("soundex", metaphone)
            .getResultList();
        return entities;
    }

    private List<Match> match(Query query, String queryText,
        List<AdminEntity> entities) {
        List<Match> matches = Lists.newArrayList();
        for (AdminEntity entity : entities) {
            MatchType type = typeOf(entity);
            if (Strings.isNullOrEmpty(query.getType())
                || type.getId().equals(query.getType())) {
    ...
    private List<PropertyDescription> queryProperties(String schema) {
        if (schema.startsWith("adminUnitLevel/")) {
            int adminLevelId = Integer.parseInt(schema
                .substring("adminUnitLevel/".length()));
    ...
    private List<PropertyDescription> queryAdminLevelProperties(int adminLevelId) {
        ...
        properties
            .add(new PropertyDescription("/admin/country_name", "Country"));

        AdminLevel level = entityManager.get().find(AdminLevel.class,
            adminLevelId);
        AdminLevel parent = level.getParent();
        while (parent != null) {
            PropertyDescription parentEntityName = new PropertyDescription(
                "/admin/level/" + parent.getId() + "/name",
                parent.getName() + " Name");
    ...
    public Viewable preview(@PathParam("id") int id) {
        return new Viewable("/preview/AdminEntity.ftl",
            new AdminEntityPreview(entityManager.get().find(AdminEntity.class,
                id)));
    }
