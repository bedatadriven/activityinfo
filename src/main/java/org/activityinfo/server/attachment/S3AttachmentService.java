...
public class S3AttachmentService implements AttachmentService {
    ...
    @Override
    public void serveAttachment(String key, HttpServletResponse resp)
        throws IOException {
    ...
    public void upload(String key, FileItem fileItem,
        ...
        metadata
            .setContentDisposition("Content-Disposition: attachment; filename=\""
                + fileItem.getName() + "\"");

        AmazonS3Client client = new AmazonS3Client(credentials);
        client.putObject(new PutObjectRequest(bucketName, key, uploadingStream,
            metadata));
    }
