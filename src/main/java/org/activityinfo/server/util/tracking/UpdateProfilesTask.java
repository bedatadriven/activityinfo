...
public class UpdateProfilesTask extends HttpServlet {
	...
	private static final Logger LOGGER = Logger.getLogger(UpdateProfilesTask.class.getName());
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
	    ...
	    MessageBuilder messageBuilder = new MessageBuilder("31eced8bff93159dfee3b10f8e3a0804");
	    MixpanelAPI mixpanelAPI = new MixpanelAPI();
		List<User> users = entityManager.get().createQuery("select u from User u").getResultList();
		for(User user : users) {
			try {
				JSONObject msg = messageBuilder.set("u" + user.getId(), propertiesOf(user));
				mixpanelAPI.sendMessage(msg);
			} catch(Exception e) {
				LOGGER.log(Level.SEVERE, "Could not update profile for " + user.getEmail(), e);
			}
		}		
	}
	...
	private Date findLastSeen(User user) {
		return (Date) entityManager.get().createQuery("select max(a.dateCreated) from Authentication a where a.user = :user")
		.setParameter("user", user)
		.getSingleResult();
	}

	private Date findCreated(User user) {
		return (Date) entityManager.get().createQuery("select min(a.dateCreated) from Authentication a where a.user = :user")
		.setParameter("user", user)
		.getSingleResult();
	}
	
	private long countCreatedDatabases(User user) {
		return (Long) entityManager.get().createQuery("select count(*) from UserDatabase db where db.owner = :user")
		.setParameter("user", user)
		.getSingleResult();
	}
	
	private String getPartner(User user) {
		List<UserPermission> perms = entityManager.get().createQuery("select p from UserPermission p where p.user = :user")
		.setParameter("user", user)
		.getResultList();
	...
	private long countAdminDatabases(User user) {
		return (Long) entityManager.get().createQuery("select count(*) from UserPermission p where p.user = :user and p.allowDesign=1")
		.setParameter("user", user)
		.getSingleResult();
	}
}
