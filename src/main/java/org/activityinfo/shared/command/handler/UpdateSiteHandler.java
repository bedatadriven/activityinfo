...
public class UpdateSiteHandler implements
    CommandHandlerAsync<UpdateSite, VoidResult> {

    @Override
    public void execute(final UpdateSite command, ExecutionContext context,
        final AsyncCallback<VoidResult> callback) {

        final Map<String, Object> changes = command.getChanges()
            .getTransientMap();
    ...
    private void updateAttributeValues(SqlTransaction tx, int siteId,
        Map<String, Object> changes) {
        for (Entry<String, Object> change : changes.entrySet()) {
            if (change.getKey().startsWith(AttributeDTO.PROPERTY_PREFIX)) {
                int attributeId = AttributeDTO.idForPropertyName(change
                    .getKey());
            ...
            .execute(tx, new SqlResultCallback() {
                ...
                public void onSuccess(SqlTransaction tx, SqlResultSet results) {
                    if (results.getRows().size() == 1) {
                        updateReportingPeriod(tx, siteId, results.getRow(0)
                            .getInt("reportingPeriodId"), changes);
                    }
                }
            });
    }
        ...
        for (Map.Entry<String, Object> change : changes.entrySet()) {
            if (change.getKey().startsWith(IndicatorDTO.PROPERTY_PREFIX)) {
                int indicatorId = IndicatorDTO
                    .indicatorIdForPropertyName(change.getKey());
