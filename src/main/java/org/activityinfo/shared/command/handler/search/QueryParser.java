...
public class QueryParser {
    ...
    public void parse(String query) {
        this.query = query;
        if (!checker.checkQuery(query)) { // Bugger out if the user fails to
                                          // enter normal queries
            hasFailed = true;
        ...
        try {
            ...
            if (hasDimensions) { // i.e. a "location:kivu" or
                                 // "activity:NFI ditribution"
                createDimensions();
                parseSearchTerms();
    ...
    /**
     * performs simple parsing on the searchterm, falling back on space
     * seperation if no comma found
     */
    ...
    private boolean isSupportedDimension(DimensionType dimension) {
        return (dimension == DimensionType.Activity ||
            ...
            dimension == DimensionType.Indicator || dimension == DimensionType.IndicatorCategory);
    }
    ...
    private void parseSearchTerms() {
        ...
        for (int dimensionIndex = 0; dimensionIndex < dimensionCount; dimensionIndex++) {
            ...
            // boolean isQuoted = query.substring(startPosSearchTerm,
            // startPosSearchTerm + 1) == quote;
            StringBuilder sb = new StringBuilder();
            boolean isQuoted = false;
            // Parse text between the colon and the starting position of the
            // next dimension
     ...
     * 
     * Assumes every character before a colon is part of the name of a
     * dimension, unless a quote, space is encountered or start of string is
     * reached
     */
